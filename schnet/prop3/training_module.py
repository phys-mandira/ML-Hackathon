...
if __name__ == "__main__":
    ...
    data_set = AtomsDataModule(
        ...
        transforms = [
            ...
            trn.RemoveOffsets('prop3', remove_mean=True),
        ...
        load_properties = ['prop3'],
        property_units = {'prop3': 'Ha'}
    )
    ...
    pred_prop3 = spk.atomistic.Atomwise(n_in = n_atom_basis, output_key = 'prop3')

    nnpot = spk.model.NeuralNetworkPotential(
            representation = schnet,
            input_modules = [pairwise_distance],
            output_modules = [pred_prop3],
            postprocessors = [trn.CastTo64(), trn.AddOffsets('prop3', add_mean=True)]
    )

    #output model parameters
    output_prop3 = spk.task.ModelOutput(
            name = 'prop3',
    ...
    calculator = spk.interfaces.SpkCalculator(
            ...
            energy_key = 'prop3',  # name of energy property in model
            energy_unit = 'Ha', # unit of the property
            device = 'cuda',    # computation device
            )


    correlation_file = os.path.join(log_data, 'testing_AccVsPred_prop3.dat')
    try:
    ...
    for i in range(500):
        ...
        prop3_acc = test_data["prop3"][i] 
        atoms = Atoms(symbols = symbols, positions = positions)

        tmp.append(prop3_acc[0])
    ...
    correlation_file = os.path.join(log_data, 'training_AccVsPred_prop3.dat')
    try:
        os.remove(corelation_file)
    except NameError:
        pass
